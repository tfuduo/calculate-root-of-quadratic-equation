...
public class Solution {
    ...
    public static double[] calculate(int a, int b, int c) {
        double m = Double.valueOf(((b * b) - (4 * a * c)));
        if (m > 1e-6) {
            return new double[]{Double.valueOf((-b - Math.sqrt(m)) / (2 * a)), Double.valueOf((-b + Math.sqrt(m)) / (2 * a))};
        } else if (m < -1e-6) {
            return new double[]{};
        } else {
            return new double[]{Double.valueOf((-b + Math.sqrt(m)) / (2 * a))};
        }
    }
}
