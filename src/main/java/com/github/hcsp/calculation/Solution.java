...
public class Solution {
    ...
    public static double[] calculate(int a, int b, int c) {
        int sqrtArgus = b * b - 4 * a * c;
        if (sqrtArgus < 0) return new double[]{};
        if (sqrtArgus == 0) return new double[]{(float)-b / (2 * a)};
        return new double[]{(-b + Math.sqrt(sqrtArgus)) / (2 * a), (-b - Math.sqrt(sqrtArgus)) / (2 * a)};
    }
}
