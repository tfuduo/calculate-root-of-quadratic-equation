...
public class Solution {
    ...
    public static double[] calculate(int a, int b, int c) {
        if ((Math.pow(b, 2) - 4 * a * c) < 0 ) return new double[] {};

        return (Math.pow(b, 2) - 4 * a * c) > 0 ? new double[] {((- b + Math.sqrt(Math.pow(b, 2) - 4 * a * c)) / 2 * a), ((- b - Math.sqrt(Math.pow(b, 2) - 4 * a * c)) / 2 * a)} : new double[]{(- b) / 2 * a};
    }
}
